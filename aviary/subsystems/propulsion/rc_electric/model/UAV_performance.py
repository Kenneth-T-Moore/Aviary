...
class Throttle(om.ExplicitComponent):
    def initialize(self):
        ...
        self.options.declare('min_current', default=0.0)
    ...
    def setup_partials(self):
        ...
        min_current = self.options['min_current']
        self.declare_partials(Dynamic.Vehicle.Propulsion.CURRENT, Dynamic.Vehicle.Propulsion.THROTTLE, val=(max_current - min_current), rows=np.arange(nn), cols=np.arange(nn))
    ...
    def compute(self, inputs, outputs):
        ...
        max_current = self.options[Aircraft.Engine.Motor.MAX_CONT_CURRENT][0]
        min_current = self.options['min_current']
        outputs[Dynamic.Vehicle.Propulsion.CURRENT] = min_current + throttle * (max_current - min_current)
