...
class PayloadRangeTest(unittest.TestCase):
    ...
    def test_payload_range(self):
        # run design case
        prob = self.prob = AviaryProblem(verbosity=1)
        copy_phase_info = deepcopy(energy_phase_info)

        copy_phase_info['post_mission']['target_range'] = (2500.0, 'nmi')
        copy_phase_info['climb']['user_options']['time_duration_bounds'] = ((20.0, 90.0), 'min')
        copy_phase_info['cruise']['user_options']['time_initial_bounds'] = ((20.0, 192.0), 'min')
        copy_phase_info['descent']['user_options']['time_duration_bounds'] = (
            (25.0, 60.0),
            'min',
        )
        prob.load_inputs(
            'models/aircraft/test_aircraft/aircraft_for_bench_FwFm.csv', copy_phase_info
        )
